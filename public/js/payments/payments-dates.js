...
const SendPaymentsMount = async (data) => {
  enviarPeticionAjax(
    `${BASE_URL}/student/setStudentPayMount`,
    "POST",
    data,
  ).done(function (data) {
    if (data.success) {
...
$(function () {
  ...
  initializeDataTable(
    "#studentPayTable",
    `${BASE_URL}/student/getStudentsPayMount`,
    [
      ...
      { data: "name", className: "text-center" },
      {
        ...
        render: function (data, type, row) {
          if (row.amount == null) {
            return (
              '<strong>No asignado</strong><br><button data-id="' +
              row.id +
              '" class="btn btn-primary btn-circle assignMount" data-bs-toggle="modal" data-bs-target="#mountModal"><i class="bi bi-plus "></i></button>'
            );
          } else {
            return "$" + row.amount;
          }
      ...
      },
      {
        ...
        render: function (data, type, row) {
          return (
            ...
            row.id +
            '" class="btn btn-primary btn-circle editStudent" data-bs-toggle="modal" data-bs-target="#StutentEditModal"><i class="bi bi-pencil-square"></i></button><button disabled id="deleteStudent" data-id="' +
            row.studentId +
            '" class="btn btn-danger btn-circle"><i class="bi bi-trash-fill"></i></button>'
          );
        },
        className: "text-center",
      },
  ...
  $("#monthAmountForm").on("submit", function (e) {
    ...
    let data = {
      amount: CashFormatInput($("#studentAmount").val()),
      studentId: $(".assignMount").data("id"),
    };
