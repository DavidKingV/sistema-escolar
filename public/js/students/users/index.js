...
$("#findMicrosoftUser").on("click", async function () {
  ...
  if (studentName === "") {
    Swal.fire({
      icon: "error",
      title: "Nombre del alumno no proporcionado",
  ...
  try {
    const response = await enviarPeticionAjax(
      `${BASE_URL}/student/findMicrosoftUser`,
      "GET",
      { studentName },
    );
    ...
    if (response.success) {
      if (response.data && response.data.success) {
        ...
        resultsDiv.append(
          '<div class="alert alert-info">No se encontraron usuarios de Microsoft para el nombre proporcionado.</div>',
        );
      }
    } else {
      Swal.fire({
        ...
        text:
          response.message ||
          "Ocurrió un error al buscar usuarios de Microsoft.",
      });
    }
  } catch (error) {
    resultsDiv.empty();
...
$("#microsoftUserSearchResults").on("submit", "form", async function (event) {
  ...
  if (
    !data.includes("microsoftId=") ||
    !data.includes("displayName=") ||
    !data.includes("mail=")
  ) {
    Swal.fire({
      icon: "error",
  ...
  confirmAlert(
    "Esta acción asignará el usuario de Microsoft al alumno seleccionado. ¿Deseas continuar?",
    "Sí, asignar usuario",
    "Cancelar",
    async () => {
      try {
        ...
        const response = await enviarPeticionAjax(
          `${BASE_URL}/student/assignMicrosoftUserToStudent`,
          "POST",
          { dataStudentUser: dataStudentUser }, // ya no necesitas {data}, la función ya lo envuelve
        );
        ...
        if (response.success) {
          Swal.fire({
            ...
            text: "El usuario de Microsoft ha sido asignado correctamente al alumno.",
          });
          ...
          $("#studentsMicrosoftUsersTable")
            .DataTable()
            .ajax.reload(null, false); // recarga la tabla sin resetear la paginación
        } else {
          Swal.fire({
            icon: "error",
            ...
            text:
              response.message ||
              "Ocurrió un error al asignar el usuario de Microsoft al alumno.",
          });
        }
      } catch (error) {
        hideLoader();
