...
function FillTable(response) {
  ...
  $("#studentGender option[value='" + response.gender + "']").prop(
    "selected",
    true,
  );
  ...
  $("#studentState option[value='" + response.civil_status + "']").prop(
    "selected",
    true,
  );
...
function initializeSubjectChangeListener(selector) {
  // Obtener la materia actualmente seleccionada
  const idSubject = $(selector).val();
