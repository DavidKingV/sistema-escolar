...
import { GetGroupsNames } from "./index.js";
...
const VerifyGroupStudent = async (studentIdGroup) => {
  try {
    const response = await $.ajax({
      url: `${BASE_URL}/student/verifyStudentGroup`,
      type: "GET",
      data: { studentIdGroup: studentIdGroup },
    });
    if (response.success) {
      if (!response.group) {
        Swal.fire({
    ...
    Swal.fire({
      ...
      title: "Error al verificar el grupo",
      text:
        "Ocurrió un error al verificar el grupo, por favor intenta de nuevo más tarde. + " +
        error,
    });
  }
};
...
const VerifyToken = async (studentId, token) => {
  try {
    const response = await $.ajax({
      url: `${BASE_URL}/student/verifyStudentToken`,
      type: "GET",
      data: { studentId: studentId, token: token },
    });
    if (response.success) {
      if (!response.token) {
        Swal.fire({
...
const GetSubjectsNames = async (carrerId) => {
  const GetSubjectSelect = async () => {
    try {
      const response = await $.ajax({
        url: `${BASE_URL}/student/getSubjectNames`,
        type: "GET",
