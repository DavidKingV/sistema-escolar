      ...
      { data: "description", className: "text-center" },
      {
        ...
        render: function (data, type, row) {
          return (
            `<button data-id="` +
            row.studentId +
            `" class="btn btn-info btn-circle addHours" data-bs-toggle="modal" data-bs-target="#addHoursModal"><i class="bi bi-plus-square"></i></button> <button data-id="` +
            row.studentId +
            `" data-total="` +
            row.total_hours +
...
const addSchedule = async (data) => {
  loadingAlert();
  try {
    sendFetch(`${BASE_URL}/api/addSchedule`, "POST", {
      scheduleData: data,
    })
      .then((response) => {
        if (!response.ok) {
