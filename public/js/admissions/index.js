...
$(function () {
  ...
  $("#newAdmissionsTable").DataTable({
    ...
    processing: true,
    ajax: {
      url: `${BASE_URL}/admission/getAllNewAdmissions`,
      type: "GET",
      dataSrc: function (data) {
        ...
        canApproveAdmissions = data.permissions.canApproveAdmissions;
        return data.data;
      },
    },
    columns: [
      {
        ...
        render: function (data, type, row) {
          if (!canApproveAdmissions) return "";
          return `
            <button class="btn btn-success btn-circle m-1 approveStudent" data-id="${row.id}">
              <i class="bi bi-check2-circle"></i>
            </button>
            <br>
            <button class="btn btn-danger btn-circle m-1 deleteApplication" data-id="${row.id}">
...
$("#newAdmissionsTable").on("click", ".approveStudent", function (e) {
  ...
  confirmAlert(
    ...
    function () {
      ...
      sendFetch(
        "https://n8n.aiethereus.com/webhook/admissionNewStudent",
        "POST",
        {
      ...
      sendFetch(`${BASE_URL}/admission/deleteAdmission`, "POST", {
        ...
        .then((data) => {
          if (data.success) {
            ...
            errorAlert(data.message || "Error al eliminar la solicitud");
          }
        })
        .catch(() => {
          errorAlert("Error al conectar con el servidor.");
